...
class ExperienceExtractor(EntityExtractor):
    ...
    def _parse_experience_block(self, text: str, in_section: bool, context: ProcessingContext) -> ExperienceRecord | None:
        ...
        confidence = self._calculate_confidence(company, job_title, start_date, in_section)
    ...
    def _extract_company(self, text: str, context: ProcessingContext) -> str | None:
        for ent in context.processed_document.named_entities:
            if ent.label == "ORG" and ent.text in text:
                return ent.text
        match = re.search(r"^\s*([A-Z][a-zA-Z0-9&, ]+(?:Inc\.|LLC|Ltd\.|Corp\.|Group|Technologies|Solutions|Company))(?=\s|$|,)", text, flags=re.MULTILINE)
        if match:
    ...
    def _extract_job_title(self, text: str) -> str | None:
        titles = [
            ...
            "Data Scientist", "Project Manager", "Developer", "Intern", 
            "Manager", "Analyst", "Architect", "Lead", "Consultant", 
            "Director", "Engineer", "Programmer"
        ]
        for title in titles:
            match = re.search(rf"\b((?:(?:Senior|Junior|Staff|Principal|Lead|Chief)\s+)?{re.escape(title)}(?:\s+Intern)?)\b", text, flags=re.IGNORECASE)
            if match:
    ...
    def _extract_description(self, text: str) -> str | None:
        lines = text.split('\n')
        desc_lines = [line for line in lines if line.strip().startswith(("-", "•", "*"))]
        if desc_lines:
    ...
    def _calculate_confidence(self, company, job_title, start_date, in_section) -> float:
        score = 0.0
        if in_section: score += 0.3
        if company: score += 0.3
        if job_title: score += 0.3
        if start_date: score += 0.1
