...
class CertificationExtractor(EntityExtractor):
    ...
    def _extract_name(self, text: str) -> str | None:
        ...
        for line in lines:
            ...
            if match:
                ...
                if 2 < len(name) < 80 and not re.match(
    ...
    def _extract_issuer(self, text: str, name: str) -> str | None:
        ...
        match = re.search(
            r"(?:Issued by|Issuer|Authority)[\s:]*([A-Za-z0-9\s.,]+)",
            text,
            re.IGNORECASE,
        )
        if match:
    ...
    def _extract_dates(self, text: str) -> tuple[str | None, str | None]:
        ...
        issue_match = re.search(
            r"(?:Issued|Earned|Date|Obtained)[\s:]*((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+\d{4}|\d{4}-\d{2}|\d{2}/\d{4}|\d{4})",
            text,
            re.IGNORECASE,
        )
        ...
        expiry_match = re.search(
            r"(?:Expires|Valid until|Expiration)[\s:]*((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+\d{4}|\d{4}-\d{2}|\d{2}/\d{4}|\d{4})",
            text,
            re.IGNORECASE,
        )
        ...
        if not issue_date and not expiry_date:
            ...
            pattern = re.compile(
                r"\b((?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+\d{4}|\d{4})\b",
                re.IGNORECASE,
            )
    ...
    def _extract_credential_id(self, text: str) -> str | None:
        match = re.search(
            r"(?:Credential ID|Cert(?:ification)? ID|License Number|ID)[\s:]*([A-Za-z0-9-]+)",
            text,
            re.IGNORECASE,
        )
        if match:
