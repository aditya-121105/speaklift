...
class JDResponsibilityExtractor(EntityExtractor):
            ...
            for sec in context.document.sections.values():
                if sec.section_type.value == "responsibilities":
                    self._extract_from_text(
                        sec.content, 1.0, extracted_dict, in_section=True
                    )
                else:
                    self._extract_from_text(
                        sec.content, 0.8, extracted_dict, in_section=False
                    )
        else:
            # No sections detected, parse whole document with lower confidence
            self._extract_from_text(
                context.document.cleaned_text, 0.8, extracted_dict, in_section=False
            )
    ...
    def _is_valid_noun_phrase(self, line: str) -> bool:
        ...
        if re.search(
            r"\b(years?|experience|freshers?|graduates?)\b", line, re.IGNORECASE
        ):
            return False
    ...
    def _add_record(
        self, description: str, confidence: float, extracted_dict: dict
    ) -> None:
