...
class JDEducationExtractor(EntityExtractor):
            ...
            for pattern_str, canonical_degree in self.DEGREE_MAPPING.items():
                ...
                if match:
                    ...
                    field_match = re.search(
                        rf"{pattern_str}(?:\'s)?\s*(?:degree)?\s*(?:of\s+[a-zA-Z]+\s*)?\s*(?:in)\s+([a-zA-Z\s]+?)(?=\b(?:or|and)\b|,|\.|$)",
                        sentence,
                        flags=re.IGNORECASE,
                    )
                    if not field_match:
                        field_match = re.search(
                            rf"{pattern_str}.*?(?:in|of)\s+([a-zA-Z\s]+?)(?=\b(?:or|and)\b|,|\.|$)",
                            sentence,
                            flags=re.IGNORECASE,
                        )
                    field_of_study = None
                    ...
                    if (
                        key not in extracted_dict
                        or confidence > extracted_dict[key].confidence
                    ):
    ...
    def _is_in_education_section(
        self, sentence: str, context: ProcessingContext
    ) -> bool:
